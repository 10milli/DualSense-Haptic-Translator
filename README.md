...
- Optional DSX output, telemetry relay, gamepad shortcuts for presets and HUD
...
- Horizon HUD Standby Hide now remains hidden through menus, loading, and
  garage transitions until wheel motion confirms an active driving session.
  Once confirmed, stopping the vehicle keeps the HUD visible.
- Fixed a tenfold slip-threshold conversion error that could cause false R2
  traction pulses during stable high-speed driving and premature brake-slip
  response.
- Slip Threshold and Slip Off End now use the legacy semantic range `0.1..5.0`
  in `0.1` steps, with runtime-matched advanced-control ranges.
- Gear Shift Kick restores Left, Right, and Both side selection. Acceleration G
  Punch pulse strength again matches the legacy output formula.
- Haptic and trigger preset changes persist only when the main `SAVE` button is
  pressed. Closing the app or saving ordinary preferences preserves the last
  explicitly saved preset content.
- Gamepad shortcut controls are more compact, support deletion with `None`, and
  include a HUD visibility shortcut.
